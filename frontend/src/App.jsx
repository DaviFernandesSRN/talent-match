    ...
    <div className={`flex h-screen overflow-hidden ${darkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
      <aside className="w-64 bg-slate-900 text-white flex flex-col hidden md:flex">
        <div className="p-6 border-b border-slate-800 flex items-center gap-3">
          <div className="w-8 h-8 bg-indigo-600 rounded flex items-center justify-center">🎯</div>
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-12">
        <div className="max-w-5xl mx-auto">
          {/* TÍTULO ATUALIZADO PARA TALENTMATCH */}
              ...
              {!result && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                  ...
                  <div className="lg:col-span-2 flex justify-end">
                    <button onClick={handleAnalyze} disabled={loading} className="px-8 py-4 rounded-xl font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600">{loading ? '⏳ Processando...' : '✨ Executar Análise'}</button>
                  </div>
                </div>
              )}
              {result && (
                <div className="space-y-6">
                  ...
                  <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border dark:text-white"><ReactMarkdown>{result.feedback}</ReactMarkdown></div>
